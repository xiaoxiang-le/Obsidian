...
| 用法         | 示例                    | 说明                                      |
| ---------- | --------------------- | --------------------------------------- |
| 1.引用当前对象   | `this.name = name;`   | 区分成员变量与参数，解决命名冲突                        |
| 2.调用本类构造器  | `this()`或`this(参数)`   | 在构造器第一行调用另一个构造器                         |
| 3.作为方法参数传递 | `visitor.visit(this)` | 将当前对象传给其他方法,注意如果不加this意味着传给的其他方法必须是无参数的 |
| 4.返回当前对象   | `return this`         | 用于链式调用                                  |
